<script lang="ts">
	...
	onMount(() => {
		const dvd = document.querySelector('.dvd') as HTMLDivElement;
		if (!dvd) return;
		...
		const animate = () => {
			if (x + width + vx > window.innerWidth || x + vx < 0) {
				vx = -vx;
				dvd.style.color = `hsl(${Math.random() * 360}, 100%, 50%)`;
			}

			if (y + height + vy > window.innerHeight || y + vy < 0) {
				vy = -vy;
				dvd.style.color = `hsl(${Math.random() * 360}, 100%, 50%)`;
			}
...
<div class="flex h-full w-full flex-grow flex-col items-center justify-center border border-black">
	<h1
		class="dvd absolute flex h-24 w-24 flex-row items-center justify-center border border-black text-4xl"
	>
		<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 187.09 82.68">
			<defs>
				<style>
					.cls-1 {
						fill: currentColor;
